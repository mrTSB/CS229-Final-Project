...
# with overlap for context. We use the full series indices [0, T_total),
# where T_total = num_time.
# Let:
#   train: indices [0, train_size)
#   val: indices [train_size - context_window, train_size + val_size)
#   test: indices [train_size + val_size - context_window, T_total)
# (This way, each segment has sufficient preceding timesteps.)
# ----------------------------------------------------------------------------
...
# The first window in this segment corresponds to global index: (train_size - context_window + context_window) = train_size.
# So, we take all windows; they correspond to targets from global train_size to train_size+val_size-forecast_horizon+1.
...
# Build test windows using indices [train_size + val_size - context_window, T_total)
...
# The first window’s target corresponds to global index = (train_size+val_size - context_window + context_window) = train_size+val_size.
...
# ----------------------------------------------------------------------------
# Define the ConvLSTM Model with additional refinement layers
# ----------------------------------------------------------------------------
...
class ConvLSTMModel(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size, padding, dropout_p=0.1):
        super(ConvLSTMModel, self).__init__()
        self.convlstm = ConvLSTM(input_channels, hidden_channels, kernel_size, padding)
        # Extra convolutional layers for refinement
        self.conv1 = nn.Conv2d(hidden_channels, 32, kernel_size=3, padding=1)
        self.relu1 = nn.ReLU()
...
def plot_emission_comparison(tensor_gt, tensor_pred, idx, extent, cmap=custom_multi_color):
    ...
    axs[0].invert_yaxis()
    im1 = axs[1].imshow(gt_img_clamped, cmap=cmap, interpolation='nearest', extent=extent, norm=norm)
...
for i in range(N_test):
    ...
    fig.savefig(os.path.join(current_dir, f"test_sample_{i}_{base_lr}_{batch_size}_{num_epochs}.png"), dpi=300)
    plt.close(fig)
