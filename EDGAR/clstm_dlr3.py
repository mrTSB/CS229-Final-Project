...
# ----------------------------------------------------------------------------
# 1) Make runs reproducible by setting seeds and controlling CuDNN behavior
# ----------------------------------------------------------------------------
SEED = 42
...
# CuDNN settings for deterministic runs
...
# Convert to Tensors
X_train_norm = X_train_norm.transpose(0,1,4,2,3)  
...
train_loader = DataLoader(
    ...
    shuffle=True,
    worker_init_fn=worker_init_fn  # ensures reproducible data loading
)
...
class ConvLSTMModel(nn.Module):
    def __init__(self, input_channels, hidden_channels, kernel_size, padding, dropout_p=0.1):
        ...
        # Increase the number of filters and add extra layers for feature refinement
...
# Increase hidden_channels from 8 to 16 in the ConvLSTM
model = ConvLSTMModel(
    input_channels=5,
    hidden_channels=16,   # increased for more representational capacity
    kernel_size=3,
    padding=1,
    dropout_p=0.1         # slightly reduced dropout to retain boundary details
).to(device)
...
# ----------------------------------------------------------------------------
# (Optional) Dynamic LR: Linear Warmup then Cosine Decay
# ----------------------------------------------------------------------------
...
for epoch in range(num_epochs):
    ...
    for batch_X, batch_Y in train_loader:
        ...
        loss.backward()
        optimizer.step()
    # (Optional) Step the scheduler after each epoch
    scheduler.step()
